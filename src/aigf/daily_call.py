...
def is_playable_speaker(participant):
    is_speaker = "userName" in participant["info"] and participant["info"]["userName"] == "Ai-GF"
...
class DailyCall(daily.EventHandler):
    ...
    def on_joined(self, data, error):
        ...
        participant_id = data['participants']['local']['id']

        ROOM_URL = os.getenv('WEB_CALL_URL')
        ...
        # bot join
        requests.post(f"{BASE_URL}/bot/join", headers=headers, json=payload)
    ...
    def send_user_audio(self):
        ...
        self._input_audio_stream = self.__audio_interface.open(
            format=pyaudio.paInt16,
            channels=NUM_CHANNELS,
            rate=SAMPLE_RATE,
            input=True,
